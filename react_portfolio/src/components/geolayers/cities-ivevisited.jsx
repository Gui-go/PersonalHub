...
const SimpleMap = () => {
  ...
  useEffect(() => {
    ...
    // Initialize vector source with a single marker
    const vectorSource = new VectorSource();
    const vectorLayer = new VectorLayer({
      source: vectorSource,
    });

    // Add a single marker for Florianópolis
    ...
    // Initialize tile layer with OSM
    const tileLayer = new TileLayer({
      source: new OSM()
    });

    // Log tile load errors
    ...
    // Clean up on unmount
    return () => {
      console.log('Cleaning up SimpleMap');
      if (map) {
