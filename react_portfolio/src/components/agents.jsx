...
const Agents = ({ content }) => {
  ...
  const AIagents = content.aiagents || []; // Fallback to empty array if content.aiagents is undefined
      ...
      <div className="bg-white rounded-lg shadow-lg overflow-hidden transform transition-all duration-300 hover:shadow-xl">
        ...
        <div className="p-6 xs:p-8 sm:p-10 md:p-12">
          ...
          {AIagents.length > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
              {AIagents.map((aiagents) => (
                <div
                  ...
                  onClick={() => navigate(aiagents.path)}
                >
                  <img
                    src={aiagents.image}
                    alt={`${aiagents.title} thumbnail`}
            ...
            <p className="text-gray-600 text-base xs:text-lg sm:text-xl">No blog aiagentss available.</p>
          )}
        </div>
      </div>
    </div>
